...
<body class="skin-default fixed-layout">
            ...
            <div class="scroll-sidebar">
                ...
                <div class="user-profile">
                    <div class="user-pro-body">
                        <div><img src="../public/source/assets/images/users/index.jpg" alt="user-img" class="img-circle" style="width:70px; height:80px; !important;">
                        </div>
                        <div class="dropdown">
                    ...
                    <ul id="sidebarnav">
                        ...
                        <li> <a class="has-arrow waves-effect waves-dark" href="javascript:void(0)"
                            ...
                            <ul aria-expanded="false" class="">
                                <li><a href="http://localhost/shopping-page-master/public/AddCategoryController/loadCategory">Category Update </a></li>
                                <li><a href="http://localhost/shopping-page-master/public/indexcontroller/index">Products Update </a></li>
                                <li><a href="http://localhost/shopping-page-master/public/NewProductController/NewProduct">New Products Addition </a></li>
    ...
    <script src="../public/source/dist/js/updateProduct.js"></script>

    <script src="../public/source/dist/js/newProduct.js"></script>
    ...
    <script>
        $(document).ready(function(){
            ...
            $('#ConfirmCateBtn').click(function(){
                let newCateInputName = $('#newCateInputName').val();
                $('.tabdDataHidden ').hide();
                // console.log(newCateInputName);
                $.ajax({
                    url : "http://localhost/shopping-page-master/public/AddCategoryController/addCategory",
                    type: 'post',
                    data:{
                        action: 'addCategory',
                        name : newCateInputName,
                    },
                    dataType : 'JSON',
                    success : function(response){
                        if(response.success){
                            let showNew = $('.tabdData-hiddendNewProd');
                            showNew.attr({id:response.id});
                            showNew.attr({'cate-id':response.id});
                            showNew.find('.tdEditStatus').find('.editCateicon').attr('edit-id',response.id);
                            showNew.find('.tdEditStatus').find('.delCateTrash').attr('trash-id',response.id)
                            showNew.find('.tdName').children().text(response.name)
                            showNew.find('.tdEditStatus').find('input').attr({"checked":'true','disabled':''})
                            showNew.show();
                        }
                    },
                    error : function(){
            ...
            $('.delCateTrash').click(function(){
                let trashCateId = $(this).attr('trash-id');
                // alert('id: ');
                $.ajax({
                    url: 'http://localhost/shopping-page-master/public/AddCategoryController/deleteCategory',
                    type : 'post',
                    data :{
                        id: trashCateId,
                        action : 'del-cate'
                    ...
                    success : function(response){
                        if(response.success){
                            let choseCate = $('#tabdData-id-'+response.id);
                            alert(' category : '+response.name+' , '+ response.message +' ('+response.action +'). ' );
                            if (response.action == 'permanently'){
                                choseCate.remove();
                            }else{
                                choseCate.find('input').attr({"checked":'false','disabled':''})
                            }
                        }else{
                            alert(' category : '+response.name+' , '+response.message+' ('+response.action +'). ' );
                        }
